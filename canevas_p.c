...
void attache_gen_parente(Genealogie g,Ident x,  Individu *aine, struct list** liens);
void detruire_liens_cadet(Genealogie g, struct list** liens);
void lecture_gen(Genealogie g, Individu* lst_aine, Chaine buff, Nat negen) ;

void attache_gen_desc(Genealogie g, Ident x, Individu* lst, struct list** liens) ;
Nat profondeur_genealogie_de(Genealogie g,Ident x) ;
...
void affiche_parente(Genealogie g, Ident x, Chaine buff) {
        ...
        struct list** liens = MALLOC(struct list*) ; // contient les lien a remettre a nul
...
void affiche_descendance(Genealogie g, Ident x, Chaine buff) {
        ...
        Nat profondeur = profondeur_genealogie_de(g,x) ;

        Individu* lst_aine = CALLOCN(Individu, profondeur-1) ; 
        
        struct list** liens = MALLOC(struct list*) ;

        attache_gen_desc(g, x, lst_aine, liens) ;

        lecture_gen(g, lst_aine, buff,profondeur-1) ;
...
Individu concat_cadet(Genealogie g, Individu a, Individu b, struct list** lien){
        ...
        while(idv->icadet != omega) idv = getByIdent(g, idv->icadet) ; // aller jusqu au dernier fils
...
void attache_gen_desc(Genealogie g, Ident x, Individu *lst, struct list** liens){ // lst est les aines
        if (x == omega) return ;
        
        Individu idv = getByIdent(g, x) ;
        // au contrario des parents on pacour tout leurs enfants dans une boucle et pon pparcour leurs enfants recursivement
        
        // on ajoute les enfant a la liste de la generationa actuelle
        if (idv->ifaine == omega) return ;
        lst[0] = concat_cadet( g, getByIdent(g, idv->ifaine),  lst[0], liens ) ; // aines->v est l emplacement de la générationa actuelle

        // apelle recursif sur les enfants
        Ident cad = idv->ifaine ;
        // si il y a des enfants, on doit utiliser le suivant du tableau des aineés pour eux. s il nbexiste pas on e créer
        while (cad != omega){
                attache_gen_desc(g, cad, lst+1, liens) ;
                cad = getByIdent(g, cad)->icadet ;
        }
}
...
Nat profondeur_genealogie_de(Genealogie g, Ident x) {
    ...
    while (enfant != omega) {
        Nat h_fils = profondeur_genealogie_de(g, enfant);
